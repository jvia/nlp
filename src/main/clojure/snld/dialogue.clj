...
(defn object* [name & data]
  (assert (even? (count data)) "Must be an even number of key-value pairs.")
  (println name)
  (println data)
  (dosync
   (alter *world* #(into % name data))))
...
(def world-db
  {:plates [{:plate :0 :state :clean :location :sink}
            {:plate :1 :state :clean :location :cupboard}
            {:plate :2 :state :clean :location :cupboard}
            {:plate :3 :state :clean :location :cupboard}
            {:plate :4 :state :clean :location :cupboard}
            {:plate :5 :state :clean :location :cupboard}
            {:plate :6 :state :clean :location :cupboard}
            {:plate :7 :state :dirty :location :sink}
            {:plate :8 :state :dirty :location :sink}
            {:plate :9 :state :dirty :location :sink}]
   :cupboard {:cur-items 6 :max-items 8}
   :sink {:cur-items 4 :max-items 6}})
